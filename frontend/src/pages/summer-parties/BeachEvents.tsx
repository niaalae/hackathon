...
import { Waves, MapPin, Calendar, ArrowRight, Sun } from 'lucide-react'
    ...
    return (
        <PageLayout>
            <div dir={isRtl ? 'rtl' : 'ltr'}>
                {/* Hero */}
                <section className="relative overflow-hidden bg-gradient-to-br from-sky-50 via-white to-cyan-50 pt-20 pb-28">
                    <div className="pointer-events-none absolute top-0 right-0 h-96 w-96 rounded-full bg-sky-100/50 blur-3xl" />
                    <div className="pointer-events-none absolute bottom-0 left-0 h-80 w-80 rounded-full bg-cyan-100/40 blur-3xl" />
                    <div className="relative mx-auto max-w-4xl px-6 text-center">
                        ...
                        <h1 className="text-4xl font-bold tracking-tight text-zinc-900 sm:text-5xl lg:text-6xl">
                            Morocco's Best
                            <span className="block text-sky-500">Beach Events</span>
                        </h1>
                        ...
                        {events.map((event) => (
                            <div key={event.title} className="group flex flex-col overflow-hidden rounded-2xl border border-zinc-100 bg-white shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl">
                                <div className="relative h-52 overflow-hidden">
                                    ...
                                    <span className={`absolute top-3 right-3 rounded-full px-2.5 py-1 text-[11px] font-bold backdrop-blur ${categoryColor[event.category] || 'bg-zinc-50 text-zinc-600'}`}>
                                        {event.category}
                                    </span>
                                </div>
                                <div className="flex flex-1 flex-col p-5">
                                    ...
                                    <button className="mt-5 inline-flex items-center gap-1.5 text-sm font-medium text-sky-500 transition hover:text-sky-600">
                ...
                <section className="mx-auto max-w-4xl px-6 pb-24">
                    <div className="rounded-3xl bg-gradient-to-r from-sky-500 to-cyan-500 p-10 text-center sm:p-14">
                        <Sun className="mx-auto mb-4 h-8 w-8 text-white/80" />
                        ...
                        <button className="mt-8 inline-flex items-center gap-2 rounded-full bg-white px-8 py-3.5 text-sm font-semibold text-sky-600 transition hover:-translate-y-0.5 hover:shadow-lg">
